...
go run ./cmd/demo -log-level debug               # demo binary; writes data/sstable (-path to change)
```
...
- `cmd/demo/main.go` is a `package main` demo, not a Go test. It writes to `-path` (default `data/sstable`, relative to the current directory), creating the directory if needed.
